package com.example.recettes.data.api

import retrofit2.http.GET
import retrofit2.http.Query

data class MealSearchResponse(val meals: List<MealDto>?)
data class MealDetailResponse(val meals: List<MealDetailDto>?)
data class CategoryResponse(val categories: List<CategoryDto>?)
...
data class MealDetailDto(
    ...
    // on recupere les ingredients non vides
    fun getIngredients(): List<Pair<String, String>> {
        ...
        return all.filter { !it.first.isNullOrBlank() }
...
interface MealApi {
    ...
    @GET("filter.php")
    suspend fun filterByCategory(@Query("c") category: String): MealSearchResponse
}
